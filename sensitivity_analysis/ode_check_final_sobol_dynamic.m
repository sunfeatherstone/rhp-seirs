...
S1_cum = zeros(P,T);     ST_cum = S1_cum;
Var_inc = var([YA;YB],0,1);          % 1×T
Var_cum = var([CA;CB],0,1);
for j = 1:P
    TC = ThetaA;
    if j<=8, TC(:,idxTheta(j)) = ThetaB(:,idxTheta(j));
    else,    TC(:,idxA)        = ThetaB(:,idxA);
    end
    ...
    S1_inc(j,:) = 1 - mean( (YB-YC).^2 ,1 ) ./ (2*Var_inc);
...
S1_cum = max(S1_cum-thr,0);  ST_cum = max(ST_cum-thr,0);
figs = {'DynS1_inc','DynST_inc','DynS1_cum','DynST_cum'};
datas = {S1_inc,ST_inc,S1_cum,ST_cum};
titles= {'S_1(t) — Inc(t)','S_T(t) — Inc(t)', ...
         'S_1(t) — Cum(t)','S_T(t) — Cum(t)'};

xt_tick_hr = 0:24*1:ceil(t_hours(end));
