...
% ------------------------------------------------------------
% Floquet multiplier at DFE for periodic backbone
% RK4 integrator, default dt = 0.05 h
% 
% INPUT
%   theta20 : vector, log-rates [ β0 σ0 … κ γ δ ] (see original code)
%   F_hour  : handle, baseline activity curve (h) -> density
%   dt      : step size in hours  (optional, default 0.05)
%
% OUTPUT
%   rho     : principal Floquet multiplier ρ_per
% ------------------------------------------------------------
if nargin < 3
    dt = 0.05;                 % 0.5h 会严重放大误差
end
...
T_tot  = 168;                       % one week, hours
nStep  = round(T_tot / dt);         % ensure integer steps

% --- pre-sample F(t) at t, t+dt/2, t+dt ---
t_vec  = (0:nStep) * dt;            % length nStep+1
F      = F_hour(t_vec);             % row vector
Fp     = F.^kappa;
meanF  = mean(Fp);                  % weekly mean after κ
Fs     = Fp / meanF;                % F_star(t)

% make mid-points for RK4
t_mid  = t_vec(1:end-1) + 0.5*dt;
F_mid  = F_hour(t_mid);
Fs_mid = (F_mid.^kappa) / meanF;    % use same meanF

% ------------ RK4 integration of Φ̇ = J Φ ------------
Phi = eye(2);                       % fundamental matrix Φ(0)=I
...
for k = 1:nStep
    ...
    K1 = J1 * Phi;
    K2 = J2 * (Phi + 0.5*dt*K1);
    K3 = J2 * (Phi + 0.5*dt*K2);    % note: J2 (mid) again
    K4 = J3 * (Phi + dt*K3);
