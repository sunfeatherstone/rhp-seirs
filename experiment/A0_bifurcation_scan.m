...
ckptFile   = 'experiment/A0_bif_scan_ckpt.mat';   
csvFile    = 'experiment/A0_bifurcation_scan_periodic_means.csv';
saveStep   = 50;                              
nTail      = 100;                             
dt_hr      = 1;                               
Tper_hr    = 168;                              
...
if isfile(ckptFile)
    load(ckptFile, 'A0_grid', 'meanI_periods', 'processed');
    fprintf('[Resume]  Processed %d / %d A₀₀\n', nnz(processed), numel(A0_grid));
else
    nGrid        = 10;
    A0_grid     =  logspace(log10(0.0001), log10(10000), nGrid);

    meanI_periods = nan(nGrid, nTail);  
    processed     = false(1, nGrid);

    save(ckptFile,'A0_grid','meanI_periods','processed','-v7.3');
    fprintf('[Init]   Create a new checkpoint: A₀=%d\n', nGrid);
end

for i = 1:nGrid
    if processed(i), continue; end

A0 = A0_grid(i);
    pulseScaled      = pulse;
    pulseScaled.A    = A0 * pulse.A;

    Y = run_full_integrate(beta0_fixed,sigma0,kappa,delta,omega,gamma, ...
                           F_vec,dt_hr,N_all,MAX_STEPS,pulse);
    I = Y(end-nTail*nPer+1:end,3); 
    I_mat = reshape(I, nPer, nTail);         
    meanI_periods(i,:) = mean(I_mat,1);      
    Imin = min(meanI_periods(i,:));
    Imax = max(meanI_periods(i,:));
    fprintf('   I range (last %d cycles):  [%.3e  –  %.3e]\n', nTail, Imin, Imax);
    processed(i) = true;
    if mod(nnz(processed),saveStep)==0 || i==nGrid
        save(ckptFile,'A0_grid','meanI_periods','processed','-v7.3');
        fprintf('   Checkpoint saved (%d/%d)\n', nnz(processed), nGrid);
    end
end

if all(processed)
    A0_col     = repelem(A0_grid.', nTail);
    period_idx = repmat((1:nTail).', nGrid, 1);
    meanI_col  = reshape(meanI_periods.', [], 1);

    T = table(A0_col, period_idx, meanI_col, ...
              'VariableNames', {'A0','period','mean_I'});

    writetable(T, csvFile);
    fprintf('◎ CSV written out:%s\n', csvFile);
    delete(ckptFile);
    fprintf('◎ All A₀ scans completed, checkpoint files deleted\n');
end
